                    ...
                    <>
                        ...
                        <View style={{height: "100%", width: "100%", position: 'absolute', top: 0, left: 0, alignItems: 'center', justifyContent: 'center'}}>
                            <View style={{padding: 10, borderRadius: 8, margin: 10, backgroundColor: currentTheme.backgroundPrimary + "dd", justifyContent: 'center', alignItems: 'center'}}>
                                ...
                                <TouchableOpacity onPress={async () => {!client.servers.get(state.inviteServer?.server_id) && await client.joinInvite(state.inviteServerCode); app.openServer(client.servers.get(state.inviteServer?.server_id)); app.openLeftMenu(true); setState({inviteServer: null, inviteServerCode: null})}} style={styles.button}><Text>{client.servers.get(state.inviteServer?.server_id) ? "Go to Server" : "Join Server"}</Text></TouchableOpacity>
                            </View>
                        </View>
                    </>
                    : 
